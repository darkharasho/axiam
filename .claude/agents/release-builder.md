...
**FORBIDDEN:** Do NOT run `npm run build`, `npm run build:linux`, `npm run build:win`, `npx electron-builder`, `npm version`, `git tag`, `git push`, or ANY other build/release command. The script in Job 2 does ALL of that. If you run any of those commands yourself, the release will be broken and incomplete.
...
- Otherwise: compute next version (e.g. 0.5.1 + minor -> 0.6.0)
...
This single script handles: version bump + commit + push -> build -> electron-builder (linux + win) -> git tag + push -> GitHub Release upload.
...
You have a persistent, file-based memory system at `/var/home/mstephens/Documents/GitHub/GW2AM/.claude/agent-memory/release-builder/`. This directory already exists — write to it directly with the Write tool (do not run mkdir or check for its existence).
